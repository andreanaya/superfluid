...
function generateFont(config) {
	...
	.on('finish',function() {
		...
		if(config.less) {
			var template = handlebars.compile(fs.readFileSync(path.resolve(config.less.template), 'utf8'));
			...
			fs.writeFile(path.resolve(config.less.output), template(data), function(err) {
				if(err) {
